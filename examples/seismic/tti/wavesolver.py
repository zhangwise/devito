# coding: utf-8
from devito import TimeFunction, Function, memoized, Dimension
from examples.seismic.tti.operators import ForwardOperator, ForwardOperatorDFT
...
class AnisotropicWaveSolver(object):
    ...
    @memoized
    def op_fwd_dft(self, nfreqs=1, kernel='shifted', save=False):
        """Cached operator for forward runs with buffered wavefield"""
        return ForwardOperatorDFT(self.model, nfreqs=nfreqs, save=save, source=self.source,
                                  receiver=self.receiver, time_order=self.time_order,
                                  space_order=self.space_order,
                                  kernel=kernel, **self._kwargs)
